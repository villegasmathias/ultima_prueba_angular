(function() {
	"use strict";
	var app = angular.module("newsApp")//si tiene los corchetes vacios estoy creando un nuevo modulo, y si no le pongo los corchetes, llama a uno creado previamente creado con ese nombre
	.controller("newsController",
		function(newsService){
		var vm = this;

		vm.news = [];//creo arreglo auxiliar por asi decir

		vm.loading = true; //creamos variable en verdadero para q muestre el spinner

		newsService
		.getAll()
		.then(function(response){
			vm.loading = false; //cuando entra se pone falso , si tuviera un buscar lo tendria q activar y cuando salga desactivarla
			if(response.status == 200){
				//vm.news = response.data.articles;//llena el arreglo con el arreglo q viene de la api//ejemplo anterior
				vm.news = [];//define arreglo vacio
				for (var i = 0 ; i < response.data.articles.length; i++)//response.data.articles.length devuelve la cantidad de articulos
				{
					if (response.data.articles[i].urlToImage!=null) //pregunto si la imagen del elemento del arreglo es distinta de null
					{
						vm.news.push(response.data.articles[i]);//push coloca la noticia en el arreglo
					}
				}
			}else {
				alert("esta todo mal");
